...
export default function TtsSection({
  ...
  // another engine don't linger (e.g. an OpenAI voice name "alloy" in Kokoro).
  useEffect(() => {
    const engine = settings.engines.tts.engine;
    const mv = settings.engines.tts.maleVoice || "";
    const fv = settings.engines.tts.femaleVoice || "";
    // Heuristic: if the value looks like an OpenAI voice name ("alloy","nova")
    // while we're on Kokoro, clear it (Kokoro expects its own voice names).
    const isOpenAiVoice = (v: string) => /^(alloy|echo|fable|onyx|nova|shimmer)$/i.test(v);
    let shouldClearM = false, shouldClearF = false;
    if (engine === "kokoro" && isOpenAiVoice(mv)) shouldClearM = true;
    if (engine === "kokoro" && isOpenAiVoice(fv)) shouldClearF = true;
  ...
  async function deployKokoro() {
    ...
    try {
      ...
      // synthesis) actually exercise Kokoro.
  ...
  async function saveTts() {
    ...
    const entry = {
      ...
      model:
        (tts.engine === "kokoro" ? tts.kokoroModel : tts.model) || undefined,
      maleVoice: tts.maleVoice || undefined,
      femaleVoice: tts.femaleVoice || undefined,
        ...
        <label className="flex-1">
          ...
          {(e.tts.engine === "kokoro" && kokoroVoices.length > 0) ? (
            <select className="select"
              ...
              {kokoroVoices.filter((vv) => vv.gender === "male" && vv.languages.some((l) => l.startsWith("en")))
                .map((vv) => (<option key={vv.name} value={vv.name}>{vv.name}</option>))}
            </select>
          ) : (
            <input className="input"
              placeholder="e.g. alloy, onyx"
              value={e.tts.maleVoice || ""}
              onChange={(v) => patchSettings({ engines: { ...e, tts: { ...e.tts, maleVoice: v.target.value } } })} />
          )}
        </label>
        <label className="flex-1">
          ...
          {(e.tts.engine === "kokoro" && kokoroVoices.length > 0) ? (
            <select className="select"
              ...
              <option value="">— default —</option>
              {kokoroVoices.filter((vv) => vv.gender === "female" && vv.languages.some((l) => l.startsWith("en")))
                .map((vv) => (<option key={vv.name} value={vv.name}>{vv.name}</option>))}
            </select>
          ) : (
            <input className="input"
              placeholder="e.g. nova, shimmer"
              value={e.tts.femaleVoice || ""}
              onChange={(v) => patchSettings({ engines: { ...e, tts: { ...e.tts, femaleVoice: v.target.value } } })} />
          )}
        </label>
      ...
      {e.tts.engine === "kokoro" && (
        <>
          <label className="block">
            ...
            >
              <option value="82m-v1.0-quantized">82m v1.0 quantized (~88 MB, official)</option>
            </select>
          </label>
